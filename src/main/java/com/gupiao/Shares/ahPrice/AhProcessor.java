 ...
 */
public class AhProcessor implements PageProcessor{
    private Site site = Site.me().setRetryTimes(3).setSleepTime(100);
    private AhSecondProcessor ahSecondProcessor=new AhSecondProcessor();
    @Override
    public void process(Page page) {
        String text=page.getRawText().substring(page.getRawText().indexOf("{"),page.getRawText().lastIndexOf("}")+1);
        String pageSize=new JsonPathSelector("$..pages").select(text);
        ...
        ahSecondProcessor.run(Integer.valueOf(pageSize));
    }
    ...
    public static void main(String[] args) {
        Spider.create(new AhProcessor())
                .addPipeline(new AhPipeline())
                .addUrl("http://nufm.dfcfw.com/EM_Finance2014NumericApplication/JS.aspx?" +
                        "type=CT&cmd=C._AHH&sty=FCABHL&sortType=C&sortRule=-1&page=2&pageSize=20&js=var%20quote_123" +
